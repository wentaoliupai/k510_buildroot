    ...
    float matching_threshold, int budget)
{
    ...
    {
        //LOG_WITH_ERROR_LEVEL("Invalid metric; must be either 'euclidean' or 'cosine'");
    }
...
NearNeighborDisMetric::~NearNeighborDisMetric()
{
    //LOG_WITH_DEBUG_LEVEL("~ NearNeighborDisMetric");
}
/*
void
...
*/
    for (TRACKER_DATA &data : tid_feats)
    {
        ...
        { //append
            ...
            {
                FEATURESS newSampleFeatures(newSize, FEATURE_LEN);
                newSampleFeatures.block(0, 0, oldSize, FEATURE_LEN) = samples[track_id];
                newSampleFeatures.block(oldSize, 0, addSize, FEATURE_LEN) = newFeatOne;
                samples[track_id] = newSampleFeatures;
            }
            else
            {
                FEATURESS newSampleFeatures(this->budget, FEATURE_LEN);
                //newSampleFeatures.block(0, 0, oldSize, FEATURE_LEN) = samples[track_id];
                if (addSize >= this->budget)
                    newSampleFeatures.block(0, 0, this->budget, FEATURE_LEN) = 
                        newFeatOne.block(addSize - this->budget, 0,this->budget , FEATURE_LEN);
                else
                {
                    newSampleFeatures.block(this->budget - addSize, 0, addSize, FEATURE_LEN) = newFeatOne;
                    newSampleFeatures.block(0, 0, this->budget - addSize, FEATURE_LEN) = 
                            samples[track_id].block(newSize-this->budget, 0, this->budget - addSize, FEATURE_LEN);
                }
                samples[track_id] = newSampleFeatures;
                ...
                //     if (addSize >= this->budget)
                //     {
                //         newSampleFeatures = newFeatOne.block(0, 0, this->budget, FEATURE_LEN);
                //     }
                //     else
                //     {
                //         newSampleFeatures.block(0, 0, this->budget - addSize, FEATURE_LEN) =
                //             samples[track_id].block(addSize - 1, 0, this->budget - addSize, FEATURE_LEN).eval();
                //         newSampleFeatures.block(this->budget - addSize, 0, addSize, FEATURE_LEN) = newFeatOne;
                //     }
                //     samples[track_id] = newSampleFeatures;
                // }
                ...
                //     if (addSize >= this->budget)
                //     {
                //         samples[track_id] = newFeatOne.block(0, 0, this->budget, FEATURE_LEN);
                //     }
                //     else
                //     {
                //         samples[track_id].block(0, 0, this->budget - addSize, FEATURE_LEN) =
                //             samples[track_id].block(addSize - 1, 0, this->budget - addSize, FEATURE_LEN).eval();
                //         samples[track_id].block(this->budget - addSize, 0, addSize, FEATURE_LEN) = newFeatOne;
                //     }
                // }
            }
        }
