...
public:
    ...
    void prepare_memory();
    void prepare_memory_output();
    void set_input(uint32_t index);
    void set_input(uint32_t index, cv::Mat& static_img);

    void set_output();
    void load_model(char *path);
    void pre_post(cv::Mat& raw_img, vector<uchar>& out_img);    //new
    void run();
    void run(cv::Mat& img);         //new
    void get_output();
    void post_process(std::vector<BoxInfo>& result, DETECTION_ROWS& detections);
    ~objectDetect();
